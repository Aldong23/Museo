...
class VisitorArchived extends Component
{
    use WithPagination;
    ...
    {
        $this->visitorInfo = Visitor::onlyTrashed()->find($id);

        $this->dispatch('open-archive');
    }

    public function restoreVisitor($id)
    {
        $visitor = Visitor::onlyTrashed()->find($id);
        if ($visitor) {
            $visitor->restore();

            flash()->success('Visitor restored successfully.');
        } else {
    ...
    public function render()
    {
        ...
        return view('livewire.admin.visitor-monitoring.visitor-archived', compact('visitors'));
    }
}
