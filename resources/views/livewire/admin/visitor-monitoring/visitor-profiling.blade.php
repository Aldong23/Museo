...
<x-slot:secHeading>Visitor Profiling</x-slot:secHeading>
<x-admin.body>
    ...
    <div class="w-full flex items-end justify-between px-2 py-4">
        <div class="flex space-x-2 border border-black p-1 rounded-md">
            <a href="{{ url('visitor-profiling') }}" class="px-4 py-2 text-gray-600 rounded-md bg-gray-200">
                Visitor
            </a>
            <a href="{{ url('visitor-archived') }}" class="px-4 py-2 text-gray-600 rounded-md">
                Archived Visitor
            </a>
        </div>
    </div>
            ...
            <x-form.table.tr>
                ...
                <x-form.table.td>
                    <div class="flex items-center gap-3">
                        <x-form.view-btn href="/visitor-profiling-view/{{ $visitor->id }}"/>
                        @if(auth()->user()->is_admin || auth()->user()->is_tourist_assistance || auth()->user()->is_technical)
                        <x-form.archive-btn  wire:click="openArchive({{ $visitor->id }})"/>
                        @endif
                    </div>
                </x-form.table.td>
            </x-form.table.tr>
    ...
    {{-- --------------------------------------------- ARCHIVE MODAL ------------------------------------- --}}
    <x-modal.archive>
        <p>Archive this Visitor?</p>

        <x-slot:button>
            @if ($visitorInfo)
                <x-form.button wire:click="archiveVisitor({{ $visitorInfo->id }})">Archive</x-form.button>
            @endif
        </x-slot:button>
    </x-modal.archive>
