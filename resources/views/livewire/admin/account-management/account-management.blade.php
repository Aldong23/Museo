...
<x-slot:secHeading>Account Management</x-slot:secHeading>
<x-admin.body>
    ...
    <div class="w-full flex items-end justify-between px-2 py-4">
        <div class="flex space-x-2 border border-black p-1 rounded-md">
            <a href="{{ url('/account-management') }}" class="px-4 py-2 text-gray-600 rounded-md bg-gray-200">
                Users
            </a>
            <a href="{{ url('/account-archived') }}" class="px-4 py-2 text-gray-600 rounded-md">
                Archived Users
            </a>
        </div>
    </div>
            ...
            <x-form.table.tr>
                ...
                <x-form.table.td>
                    <div class="flex items-center gap-1">
                        <x-form.view-btn wire:click="openView({{ $user->id }})" />
                        <x-form.edit-btn href="/account-management-edit/{{ $user->id }}" />
                        <x-form.archive-btn wire:click="openArchive({{ $user->id }})" />
                    </div>
                </x-form.table.td>
            </x-form.table.tr>
        @endforeach
    ...
    {{-- --------------------------------------------- VIEW MODAL ------------------------------------- --}}
    <x-modal.view header="Account Details">
        @if ($userInfo)
            <div class="w-full h-full flex items-center justify-center">
                <div class="text-lg font-sans">
                    <p> <span class="text-gray-500">Name: </span>
                        {{ $userInfo->fname . ' ' . $userInfo->mname . ' ' . $userInfo->lname . ' ' . $userInfo->suffix }}
                    </p>
                    <p> <span class="text-gray-500">Position: </span>
                        {{ $userInfo->position }}
                    </p>
                    <p> <span class="text-gray-500">Employee No.: </span>
    ...
    {{-- --------------------------------------------- ARCHIVE MODAL ------------------------------------- --}}
    <x-modal.archive>
        <p>Archive this Account?</p>

        <x-slot:button>
            @if ($userInfo)
                <x-form.button wire:click="archiveAccount({{ $userInfo->id }})">Archive</x-form.button>
            @endif
        </x-slot:button>
    </x-modal.archive>
</x-admin.body>
