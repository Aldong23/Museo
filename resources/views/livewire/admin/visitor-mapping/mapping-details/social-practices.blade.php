<x-layouts.visitor-layout>
    <div class="relative w-full p-10 flex flex-col gap-6">
        <x-visitor-view.row>
            @isset($social->cultural_heritage_category)
                <x-visitor-view.text label="Cultural Heritage Category" content="{{ $social->cultural_heritage_category }}" />
            @endisset

            @isset($social->cultural_heritage_type)
                <x-visitor-view.text label="Types of Cultural Heritage" content="{{ $social->cultural_heritage_type }}" />
            @endisset
        </x-visitor-view.row>

        @isset($social->name)
            <x-visitor-view.text label="Name of the Elements" content="{{ $social->name }}" />
        @endisset

        <x-visitor-view.section label="Featured Collections">
            @isset($social->photo_details)
                @foreach ($social->photo_details as $photo)
        ...
        <x-visitor-view.section label="Background Information">
            ...
            <x-visitor-view.row>
                @isset($social->type)
                    <x-visitor-view.text label="Type" content="{{ $social->type }}" />
                @endisset
            </x-visitor-view.row>


            @isset($social->geographical_location)
                <x-visitor-view.text label="Geographical Location and Range of the Elements"
                    content="{{ $social->geographical_location }}" />
            @endisset

            @isset($social->related_domains)
                <x-visitor-view.text label="Related Domains of the Intangible Cultural Heritage"
                    content="{{ $social->related_domains }}" />
            @endisset

        </x-visitor-view.section>

        <x-visitor-view.section label="Description of Intangible Cultural Heritage">
            @isset($social->summary_of_elements)
                <x-visitor-view.text label="Summary of Elements" content="{{ $social->summary_of_elements }}" />
            @endisset
        </x-visitor-view.section>

        @isset($social->list_of_tangible_movable_heritage)
            <x-visitor-view.section
                label="List of Significant Tangible Movable Heritage Used / Associated with the Element">
                @foreach ($social->list_of_tangible_movable_heritage as $list)
                    <div class="w-full p-4 space-y-4 border border-gray-300 rounded-lg">
                        <x-visitor-view.row>
                            @isset($list['name_1'])
        ...
        @isset($social->stories)
            <x-visitor-view.text label="Stories / Narratives Associated with the Element"
                content="{{ $social->stories }}" />
        @endisset

        @isset($social->significance)
            <x-visitor-view.text label="Significance" content="{{ $social->significance }}" />
        @endisset

        <x-visitor-view.section label="Assessment of the Practice">
            @isset($social->assessment_of_practice)
                <x-visitor-view.text label="" content="{{ $social->assessment_of_practice }}" />
            @endisset

            @isset($social->measures_and_description_dropdown)
                <x-visitor-view.text label="Measures and Description of Safeguard Measure Taken"
                    content="{{ $social->measures_and_description_dropdown }}" />
            @endisset

            @isset($social->measures_and_description_text)
                <x-visitor-view.text label="" content="{{ $social->measures_and_description_text }}" />
            @endisset
        </x-visitor-view.section>

        <x-visitor-view.section label="References">
            @isset($social->supporting_documentation)
                <x-visitor-view.text label="Supporting Documentation"
                    content="{{ implode(', ', $social->supporting_documentation) }}" />
            @endisset

            @isset($social->key_informat)
                <x-visitor-view.text label="Key Informat/s" content="{{ $social->key_informat }}" />
            @endisset

            @isset($social->name_of_mapper)
                <x-visitor-view.text label="Name of Mapper" content="{{ $social->name_of_mapper }}" />
            @endisset

            @isset($social->date_profiled)
                <x-visitor-view.text label="Date Profiled" content="{{ $social->date_profiled->format('F j, Y') }}" />
            @endisset
        </x-visitor-view.section>

        <x-visitor-view.section label="Attachment">
            @isset($social->attachment_text)
                <x-visitor-view.text label="Supporting Documentation" content="{{ $social->attachment_text }}" />
            @endisset

            @isset($social->attachment_image)
                <x-visitor-view.image label="">
                    <img src="{{ asset('storage/' . $social->attachment_image) }}" alt="{{ $social->attachment_image }}"
                        class="photo-img clickable-img w-auto h-60">
                </x-visitor-view.image>
            @endisset
        </x-visitor-view.section>
