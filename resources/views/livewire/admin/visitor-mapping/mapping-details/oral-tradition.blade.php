<x-layouts.visitor-layout>
    <div class="relative w-full p-10 flex flex-col gap-6">
        <x-visitor-view.row>
            @isset($oral->cultural_heritage_category)
                <x-visitor-view.text label="Cultural Heritage Category" content="{{ $oral->cultural_heritage_category }}" />
            @endisset

            @isset($oral->cultural_heritage_type)
                <x-visitor-view.text label="Types of Cultural Heritage" content="{{ $oral->cultural_heritage_type }}" />
            @endisset
        </x-visitor-view.row>

        @isset($oral->name)
            <x-visitor-view.text label="Name of the Elements" content="{{ $oral->name }}" />
        @endisset

        <x-visitor-view.section label="Featured Collections">
            @isset($oral->photo_details)
                @foreach ($oral->photo_details as $photo)
        ...
        <x-visitor-view.section label="Background Information">
            ...
            <x-visitor-view.row>
                @isset($oral->type)
                    <x-visitor-view.text label="Type" content="{{ $oral->type }}" />
                @endisset
            </x-visitor-view.row>


            @isset($oral->geographical_location)
                <x-visitor-view.text label="Geographical Location and Range of the Elements"
                    content="{{ $oral->geographical_location }}" />
            @endisset

            @isset($oral->related_domains)
                <x-visitor-view.text label="Related Domains of the Intangible Cultural Heritage"
                    content="{{ $oral->related_domains }}" />
            @endisset

        </x-visitor-view.section>

        <x-visitor-view.section label="Description of Intangible Cultural Heritage">
            @isset($oral->summary_of_elements)
                <x-visitor-view.text label="Summary of Elements" content="{{ $oral->summary_of_elements }}" />
            @endisset
        </x-visitor-view.section>

        @isset($oral->list_of_tangible_movable_heritage)
            <x-visitor-view.section
                label="List of Significant Tangible Movable Heritage Used / Associated with the Element">
                @foreach ($oral->list_of_tangible_movable_heritage as $list)
                    <div class="w-full p-4 space-y-4 border border-gray-300 rounded-lg">
                        <x-visitor-view.row>
                            @isset($list['name_1'])
        ...
        @isset($oral->stories)
            <x-visitor-view.text label="Stories / Narratives Associated with the Element" content="{{ $oral->stories }}" />
        @endisset

        @isset($oral->significance)
            <x-visitor-view.text label="Significance" content="{{ $oral->significance }}" />
        @endisset

        <x-visitor-view.section label="Assessment of the Practice">
            @isset($oral->assessment_of_practice)
                <x-visitor-view.text label="" content="{{ $oral->assessment_of_practice }}" />
            @endisset

            @isset($oral->measures_and_description_dropdown)
                <x-visitor-view.text label="Measures and Description of Safeguard Measure Taken"
                    content="{{ $oral->measures_and_description_dropdown }}" />
            @endisset

            @isset($oral->measures_and_description_text)
                <x-visitor-view.text label="" content="{{ $oral->measures_and_description_text }}" />
            @endisset
        </x-visitor-view.section>

        <x-visitor-view.section label="References">
            @isset($oral->supporting_documentation)
                <x-visitor-view.text label="Supporting Documentation"
                    content="{{ implode(', ', $oral->supporting_documentation) }}" />
            @endisset

            @isset($oral->key_informat)
                <x-visitor-view.text label="Key Informat/s" content="{{ $oral->key_informat }}" />
            @endisset

            @isset($oral->name_of_mapper)
                <x-visitor-view.text label="Name of Mapper" content="{{ $oral->name_of_mapper }}" />
            @endisset

            @isset($oral->date_profiled)
                <x-visitor-view.text label="Date Profiled" content="{{ $oral->date_profiled->format('F j, Y') }}" />
            @endisset
        </x-visitor-view.section>

        <x-visitor-view.section label="Attachment">
            @isset($oral->attachment_text)
                <x-visitor-view.text label="Supporting Documentation" content="{{ $oral->attachment_text }}" />
            @endisset

            @isset($oral->attachment_image)
                <x-visitor-view.image label="">
                    <img src="{{ asset('storage/' . $oral->attachment_image) }}" alt="{{ $oral->attachment_image }}"
                        class="photo-img clickable-img w-auto h-60">
                </x-visitor-view.image>
            @endisset
        </x-visitor-view.section>
