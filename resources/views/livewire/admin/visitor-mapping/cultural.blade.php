<div class="w-full h-full p-10">
    <div class="w-full md:flex justify-between items-center px-5 space-y-6 md:space-x-0">
        <h1 class="text-2xl lg:text-4xl font-bold text-center md:text-start">Cultural Institutions</h1>
        <x-form.search wire:model.live="search" />
    </div>
    <br><br>

    @foreach ($institutions as $type => $items)
        @if ($items->count())
            <x-form.table.table class="border-none">
                ...
                @foreach ($items as $item)
                    <x-form.table.tr>
                        <x-form.table.td>
                            <a class="block" href="{{ route('cul.ins.details', $item->id) }}">{{ $item->name }}</a>
                        </x-form.table.td>
                    </x-form.table.tr>
                @endforeach
            </x-form.table.table>
