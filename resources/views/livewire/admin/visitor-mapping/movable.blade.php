<div class="w-full h-full p-10">
    <div class="w-full md:flex justify-between items-center px-5 space-y-6 md:space-x-0">
        <h1 class="text-2xl lg:text-4xl font-bold text-center md:text-start">Tangible Movable Cultural Heritage</h1>
            ...
            @if ($type === 'Ethnographic Object')
                <x-form.table.table class="border-none">
                    ...
                    @foreach ($items as $item)
                        <x-form.table.tr>
                            <x-form.table.td>
                                <a class="block" href="{{ route('ethno.details', $item->id) }}">{{ $item->name }}</a>
                            </x-form.table.td>
                        </x-form.table.tr>
                    @endforeach
                </x-form.table.table>
            @endif
            @if ($type === 'Archival Holdings')
                <x-form.table.table class="border-none">
                    <x-slot:head>
                        <x-form.table.th>{{ $type }}</x-form.table.th>
                    </x-slot:head>
                    @foreach ($items as $item)
                        <x-form.table.tr>
                            <x-form.table.td>
                                <a class="block"
                                    href="{{ route('archival.details', $item->id) }}">{{ $item->name }}</a>
                            </x-form.table.td>
                        </x-form.table.tr>
                    @endforeach
                </x-form.table.table>
