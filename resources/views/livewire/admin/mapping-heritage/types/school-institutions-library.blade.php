<div class="w-full h-full flex flex-col pb-8">
    ...
    <h1 class="font-bold text-xl my-2">Narrative Description</h1>
    {{-- <x-form.textarea label="Narrative Description" wire:model="narrative_description" />
    @error('narrative_description')
        <x-form.error> {{ $message }} </x-form.error>
    @enderror --}}

    <h1 class="font-bold my-2">Organizational Chart</h1>

    @if (!$organizational)
        <!-- Display file upload input when no images are uploaded -->
        <x-form.file wire:model="organizational" multiple accept="image/*" />
    @else
        <div>
            @if ($organizational)
                <!-- Display uploaded images -->
                <div class="flex flex-wrap gap-2 p-2 border border-gray-400 rounded-lg">
                    @foreach ($organizational as $organization)
                        <img class="rounded-md h-64" src="{{ $organization->temporaryUrl() }}">
                    @endforeach
                </div>
            @endif
        </div>
    ...
    <x-form.textarea label="Description" wire:model="narrative_description" />
    @error('narrative_description')
        <x-form.error> {{ $message }} </x-form.error>
    @enderror
    ...
    <x-form.textarea label="Assessment of the Institution" wire:model="assessment" />
    @error('assessment')
        <x-form.error> {{ $message }} </x-form.error>
    @enderror
